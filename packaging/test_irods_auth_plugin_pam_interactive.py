...
@unittest.skipIf(test.settings.RUN_IN_TOPOLOGY, 'insecure_mode cannot be configured for all servers from the tests.')
class test_expire_key_in_irodsA(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		self.admin = session.mkuser_and_return_session('rodsadmin', 'otherrods', 'rods', lib.get_hostname())
		...
		# Preserve the original password_min_time value and then set it to a value that's more appropriate for testing.
		self.grid_configuration_namespace = 'authentication'
		self.password_min_time_option_name = 'password_min_time'
		self.original_password_min_time = self.admin.assert_icommand([
			'iadmin',
			'get_grid_configuration',
			self.grid_configuration_namespace,
			self.password_min_time_option_name],
			'STDOUT')[1].strip()
		self.password_min_time = 5
		self.admin.assert_icommand([
			'iadmin',
			'set_grid_configuration',
			self.grid_configuration_namespace,
			self.password_min_time_option_name,
			str(self.password_min_time)])
		...
		# Get the server_config contents...
		server_config_path = paths.server_config_path()
		with open(server_config_path, 'r') as f:
			self.server_config_contents_backup = server_config_contents = json.load(f)

		# Set the insecure_mode value to true in the server configuration for easier testing.
		server_config_contents['plugin_configuration']['authentication']['pam_interactive'] = {
			'insecure_mode': True
		}

		# Write the configuration back out to the file and reload the configuration.
		with open(server_config_path, 'w') as f:
			f.write(json.dumps(server_config_contents, sort_keys=True, indent=4, separators=(',', ': ')))

		IrodsController().reload_configuration()

	@classmethod
	def tearDownClass(self):
		# Reset the grid configuration value that was modified in setUpClass.
		self.admin.assert_icommand([
			'iadmin',
			'set_grid_configuration',
			self.grid_configuration_namespace,
			self.password_min_time_option_name,
			self.original_password_min_time])
		...
		# Run iinit again to make sure we are authenticated so that the session can be torn down.
		self.auth_session.assert_icommand(['iinit'], 'STDOUT', input=f'{self.auth_session.password}\n')
		self.auth_session.__exit__()
		...
		# Put the original server_config contents back.
		with open(paths.server_config_path(), 'w') as f:
			f.write(json.dumps(self.server_config_contents_backup, sort_keys=True, indent=4, separators=(',', ': ')))

		IrodsController().reload_configuration()

	def test_authentication_error_does_not_occur_on_expiration(self):
		# First, successfully authenticate and try running some iCommand to ensure the session is working.
		self.auth_session.assert_icommand(
			['iinit'], 'STDOUT', 'Password:', input=f'{self.auth_session.password}\n')
		self.auth_session.assert_icommand(['ils'], 'STDOUT', self.auth_session.session_collection)

		# Wait for the password to expire in the client-side plugin...
		time.sleep(self.password_min_time + 1)

		# Ensure that running the non-iinit iCommand requires re-authenticating with PAM, but does not show an error.
		self.auth_session.assert_icommand(
			['ils'], 'STDOUT', ['Password:', self.auth_session.session_collection], input=f'{self.auth_session.password}\n')
